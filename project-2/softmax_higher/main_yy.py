...
parser.add_argument('--orient', required=False, default=9, type=int, help='orientations for hog')
parser.add_argument('--ppc', required=False, default=8, type=int, help='pixel per cell for hog')
parser.add_argument('--cpb', required=False, default=3, type=int, help='cell per block for hog')
...
def cifar10(root: str, use_hog=True, orient=9, ppc=8, cpb=3) -> (np.array, np.array, np.array, np.array):
    '''
    ...
    if use_hog:
        ...
        for i in tqdm(range(train_data.shape[0])):
            train_features.append(hog(train_data[i], orientations=orient, pixels_per_cell=(ppc, ppc), cells_per_block=(cpb, cpb), multichannel=True))
        ...
        for i in tqdm(range(test_datas.shape[0])):
            test_features.append(hog(test_datas[i], orientations=orient, pixels_per_cell=(ppc, ppc), cells_per_block=(cpb, cpb), multichannel=True))
...
train_datas, train_labels, test_datas, test_labels = cifar10(args.dir, args.use_hog, args.orient, args.ppc, args.cpb)
...
plt.plot(x, acc)
plt.title('{}-{}-{}-{}-{}-{}-{}-{}.jpg'.format(args.method, args.maxk, 'HOG' if args.use_hog else 'NO_HOG', \
                                           args.orient if args.use_hog else 0, args.ppc if args.use_hog else 0, \
                                                                                        args.cpb if args.use_hog else 0,\
                                                 max(acc), acc.index(max(acc))))
plt.savefig('{}-{}-{}-{}-{}-{}-{}-{}.jpg'.format(args.method, args.maxk, 'HOG' if args.use_hog else 'NO_HOG', \
                                           args.orient if args.use_hog else 0, args.ppc if args.use_hog else 0, \
                                                                                        args.cpb if args.use_hog else 0,\
                                                 max(acc), acc.index(max(acc))))

print('max accuracy: {} bestk :{}'.format(max(acc), acc.index(max(acc))))
